// =============================================================================
// Guild Quest — Edge Function « daily-tick »
//
// Fichier GÉNÉRÉ à partir de supabase/functions/daily-tick/ : ne le modifie pas
...
// function → nomme-la exactement « daily-tick » → colle tout ce fichier.
// =============================================================================
...
function companionDailyDelta(participationRate) {
...
// supabase/functions/daily-tick/index.ts
var CATCH_UP_DAYS = 14;
var SYNC_DAYS = 3;
Deno.serve(async (req) => {
  ...
  if (!expected) {
    return jsonResponse({ error: "CRON_SECRET non configur\xE9" }, 500);
  }
  if (req.headers.get("x-cron-secret") !== expected) {
    return jsonResponse({ error: "non autoris\xE9" }, 401);
  }
  const supabase = adminClient();
  const report = { usersSynced: 0, guildsTicked: 0, errors: [] };
  const { data: connectedUsers } = await supabase.from("health_connections").select("user_id");
  const userIds = [...new Set((connectedUsers ?? []).map((row) => row.user_id))];
  ...
  const { data: guilds } = await supabase.from("guilds").select("id, companion_vitality, companion_last_tick_date");
  ...
  for (const guild of guilds ?? []) {
    try {
      ...
      const startFrom = lastTick ? shiftDate(lastTick, 1) : shiftDate(yesterday, -(CATCH_UP_DAYS - 1));
      if (startFrom > yesterday) continue;
      const { data: members } = await supabase.from("guild_members").select("user_id").eq("guild_id", guild.id);
      const memberIds = (members ?? []).map((m) => m.user_id);
      if (memberIds.length === 0) continue;
      const { data: activity } = await supabase.from("daily_activity").select("activity_date, participated").in("user_id", memberIds).gte("activity_date", startFrom).lte("activity_date", yesterday).eq("participated", true);
      const participantsByDate = /* @__PURE__ */ new Map();
      ...
      await supabase.from("guilds").update({ companion_vitality: vitality, companion_last_tick_date: yesterday }).eq("id", guild.id);
